...
| ウィンドウ位置 | X, Y座標 |
| ウィンドウサイズ | 幅、高さ |
| タブ情報 | 各タブのフォルダパス、選択ファイル、フィルターインデックス |
...
ツールバーのボタン群とファイルパス表示の間の空きスペースに、最近開いたファイルへのクリック可能なリンクを最大5件表示する。ワンクリックで素早くファイルに戻れるクイックアクセス機能。

### F21との関係

- **データソース共有**: F21の`SessionManager.get_recent_files()`を使用し、表示件数のみ5件に制限
- **用途の違い**: F21はフルページリスト表示（10件）、F22はツールバーのコンパクトなリンク表示（5件）

### 表示位置

```
[ツールバーボタン群] | [履歴リンク1] › [履歴リンク2] › ... ←spacer→ [パスラベル]
```

- Help ボタンの右側、パスラベルの左側
- 区切り文字 `›` で各リンクを区別
...
| メソッド | 説明 |
|---------|------|
| `_update_history_bar()` | 履歴バーのウィジェットを再構築 |
| `_on_history_link_clicked(file_path)` | 履歴リンクのクリックハンドラ |

### FIFO動作

- 6件目のファイルを開くと、最も古い（右端の）リンクが消える
