...
現在表示中のファイルの統計・メタデータを左パネル下部のインスペクターパネルに表示。折りたたみ可能なセクション（`CollapsibleSection`）で構成。

### セクション構成

| セクション | 初期状態 | 折りたたみ時サマリー | 内容 |
|-----------|---------|-------------------|------|
| File Info | 折りたたみ | 更新日時（例: `2026-02-07 15:30`） | ファイルメタデータ |
| Stats | 折りたたみ | 行数（例: `1,234 lines`） | 統計情報 |
| Quick Actions | 常に展開 | - | 操作ボタン |
...
各セクションは折りたたんだ状態でも、ヘッダーに要約値を表示する:

- **File Info**: `File Info: 2026-02-07 15:30  ▶`
- **Stats**: `Stats: 1,234 lines  ▶`
- ファイル未選択時はサマリーなし（タイトルのみ表示）
